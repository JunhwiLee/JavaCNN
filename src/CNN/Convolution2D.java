...
public class Convolution2D {
	...
	public double[][][] pooling(double[][][] input){
        ...
        public double[][][] backward(double[] gradOutput, double learningRate){
                double[][][] grad = globalPoolingBackward(gradOutput, lastConvOutput);
                grad = convolutionLayers[convolutionLayers.length - 1].backward(lastInputs[convolutionLayers.length - 1], grad, learningRate);
        ...
        private double[][][] poolingBackward(double[][][] gradOutput, double[][][] input){
                int depth = input.length;
                int height = input[0].length;
                int width = input[0][0].length;
                ...
                for(int d = 0; d < depth; d++) {
                        for(int y = 0; y < gradOutput[0].length; y += stride) {
                                for(int x = 0; x < gradOutput[0][0].length; x += stride) {
                                        double delta = gradOutput[d][y][x] / (kernel * kernel);
                                        for(int i = 0; i < kernel; i++) {
                                                for(int j = 0; j < kernel; j++) {
                                                        int inY = y + i;
                                                        int inX = x + j;
        ...
        private double[][][] globalPoolingBackward(double[] gradOutput, double[][][] input){
                int depth = input.length;
                int height = input[0].length;
                int width = input[0][0].length;
                double[][][] gradInput = new double[depth][height][width];
                for(int d = 0; d < depth; d++) {
                        double delta = gradOutput[d] / (height * width);
                        for(int y = 0; y < height; y++) {
                                for(int x = 0; x < width; x++) {
